...
locals {
  ...
  partition = yamldecode(file(find_in_parent_folders("/${local.global.provider}/${local.global.activeName}/region.yaml")))
}
...
inputs = {
  ...
  provider_aws_eks_cluster_name = "${local.partition.name}-${local.region.region}"
}
